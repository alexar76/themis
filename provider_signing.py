...
class ProviderSigner:
    ...
    def seed(self) -> bytes:
        ...
        Handed to ``oracle_core.Signer`` in memory (see themis.federation) so the federation
        manifest is signed by the SAME identity as an audit report. Two keypairs would make
        the manifest advertise one public key while capability.json and the report signature
        advertise another — a verifier following the chain would stop there.
        """

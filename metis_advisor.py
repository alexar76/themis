...
class MemoryJobStore:
    """Single-process status store. Fast, and correct only for one replica.

    Job identifiers are unguessable, so a second replica does not leak anything
    by returning 404 — it simply cannot see a job it never accepted. That is
    fine behind a single container and wrong behind a load balancer, which is
    why the sqlite store exists.
    """
    ...
    def insert(self, job_id: str, created_at: float, state: dict[str, Any]) -> None:
        self._jobs[job_id] = _Job(created_at, state)

    def update(self, job_id: str, state: dict[str, Any]) -> None:
    ...
    def fetch(self, job_id: str) -> dict[str, Any] | None:
        job = self._jobs.get(job_id)
        return dict(job.state) if job is not None else None
...
class SqliteJobStore:
    """Status store on a shared volume, so any replica can serve any poll.

    Wall-clock timestamps, not monotonic ones: two processes cannot compare
    monotonic clocks, and the TTL has to mean the same thing on both. Writes are
    single-row and serialised by SQLite itself; WAL keeps a poll from blocking
    behind a job update.
    """
    ...
    def __init__(self, path: str) -> None:
        ...
        self._db.execute(
            "CREATE TABLE IF NOT EXISTS metis_jobs ("
            "job_id TEXT PRIMARY KEY, created_at REAL NOT NULL, state TEXT NOT NULL)"
        )
    ...
    def count(self) -> int:
        row = self._db.execute("SELECT COUNT(*) FROM metis_jobs").fetchone()
        return int(row[0]) if row else 0

    def insert(self, job_id: str, created_at: float, state: dict[str, Any]) -> None:
        self._db.execute(
            "INSERT OR REPLACE INTO metis_jobs (job_id, created_at, state) VALUES (?, ?, ?)",
            (job_id, created_at, json.dumps(state, separators=(",", ":"))),
        )

    def update(self, job_id: str, state: dict[str, Any]) -> None:
        self._db.execute(
            "UPDATE metis_jobs SET state = ? WHERE job_id = ?",
            (json.dumps(state, separators=(",", ":")), job_id),
        )

    def fetch(self, job_id: str) -> dict[str, Any] | None:
        row = self._db.execute(
            "SELECT state FROM metis_jobs WHERE job_id = ?", (job_id,)
        ).fetchone()
        if row is None:
            return None
        try:
    ...
    def prune(self, cutoff: float) -> None:
        self._db.execute("DELETE FROM metis_jobs WHERE created_at < ?", (cutoff,))
...
def build_job_store(
    *, kind: str = "", path: str = "", replicas: int = 1, clock: Callable[[], float] = time.monotonic
) -> MemoryJobStore | SqliteJobStore:
    """Choose a store, and refuse the combination that silently loses polls."""
    resolved = (kind or ("sqlite" if path else "memory")).strip().lower()
    if resolved not in {"memory", "sqlite"}:
        raise ValueError("METIS_JOB_STORE must be memory or sqlite")
    if resolved == "sqlite":
        return SqliteJobStore(path or "metis-jobs.sqlite3")
    if replicas > 1:
        raise RuntimeError(
            "METIS_JOB_STORE=memory cannot serve THEMIS_REPLICAS>1: a poll routed to "
            "another replica would 404. Set METIS_JOB_DB to a shared volume path."
        )
    return MemoryJobStore(clock)
...
class VerificationQueue:
    def __init__(
        ...
        *,
        ...
        store: MemoryJobStore | SqliteJobStore | None = None,
    ) -> None:
        ...
        self.clock = clock
        self.store = store if store is not None else MemoryJobStore(clock)
    ...
    def from_env(cls) -> "VerificationQueue":
        return cls(
            ...
            store=build_job_store(
                kind=os.getenv("METIS_JOB_STORE", ""),
                path=os.getenv("METIS_JOB_DB", ""),
                replicas=_bounded_int(os.getenv("THEMIS_REPLICAS", "1"), 1, 1, 256),
            ),
        )

    def _prune_locked(self) -> None:
        self.store.prune(self.store.now() - self.ttl_seconds)
    ...
    async def submit(self, prompt: str) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            # hard process bound.
            if self.store.count() >= self.max_jobs or len(self._tasks) >= self.max_jobs:
                return {"status": "unavailable", "reason": "verification_queue_full"}
            job_id = secrets.token_urlsafe(18)
            state = {"status": "pending"}
            self.store.insert(job_id, self.store.now(), state)
    ...
    async def _run(self, job_id: str, prompt: str) -> None:
        ...
        async with self._lock:
            self.store.update(job_id, result)

    async def get(self, job_id: str) -> dict[str, Any] | None:
        async with self._lock:
            self._prune_locked()
            state = self.store.fetch(job_id)
            return self._public(job_id, state) if state is not None else None

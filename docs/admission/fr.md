...
**Liens :** [Community supply security](https://github.com/alexar76/aimarket-hub/blob/main/docs/supply-security.md) · [Onboarding fournisseur](https://github.com/alexar76/aicom/blob/main/docs/provider-onboarding.md) · [Votre hub (fédération)](https://github.com/alexar76/aicom/blob/main/docs/join-the-federation.fr.md) · [Tutoriel THEMIS](https://github.com/alexar76/create-aimarket-agent/blob/main/docs/tutorials/themis.fr.md) · [Agent de référence](https://github.com/alexar76/themis)
...
| **Vendre en invité** sur le Hub d’un autre | publish + caution ≈ **$25** + signatures (+ THEMIS si activé) | Oui — tableau ci-dessus. |
| **Lancer votre Hub** et rejoindre la fédération | `POST /ai-market/v2/federation/announce` + sandbox assay ; le catalogue est crawlé | **Pas** de caution d’invité. Vous êtes **peer**. [Rejoindre la fédération](https://github.com/alexar76/aicom/blob/main/docs/join-the-federation.fr.md). |
| **MCP privé** uniquement sur votre ARGUS | allow-list / config MCP de **votre** agent | Politique locale — **pas** l’admission catalogue Hub. |

Un agent brillant sur GitHub peut **consommer** AIMarket via MCP/ARGUS sans THEMIS. Entrer dans un catalogue partagé pour être payé par des inconnus est le chemin dur — et ce sont **deux portes**, pas une :

- **Éditeur invité** sur un Hub existant (cette page) : jeton opérateur + caution slashable, prod ≈ **$25** (`POST /ai-market/v2/supply/stake`). Les 25 $ sont une caution, pas des frais de listing.
- **Votre propre Hub** comme peer de la fédération : [rejoindre la fédération](https://github.com/alexar76/aicom/blob/main/docs/join-the-federation.fr.md). L’annonce est gratuite ; l’admission note ce que le hub *fait*. Ce n’est pas une ligne d’invité gratuite sur le catalogue d’un autre.

Acheter des cap déjà listées ne demande ni Hub ni les 25 $.

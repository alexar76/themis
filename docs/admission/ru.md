...
**Связанное:** [Community supply security](https://github.com/alexar76/aimarket-hub/blob/main/docs/supply-security.md) · [Онбординг провайдера](https://github.com/alexar76/aicom/blob/main/docs/provider-onboarding.md) · [Свой хаб (федерация)](https://github.com/alexar76/aicom/blob/main/docs/join-the-federation.ru.md) · [Туториал THEMIS](https://github.com/alexar76/create-aimarket-agent/blob/main/docs/tutorials/themis.ru.md) · [Эталонный агент](https://github.com/alexar76/themis)
...
| **Продавать как гость** в чужой каталог (чужие платят за invoke) | publish + залог ≈ **$25** + подписи (+ THEMIS, если включён) | Да — таблица выше. |
| **Поднять свой Hub** и войти в федерацию | `POST /ai-market/v2/federation/announce` + sandbox assay; каталог забирает crawler | **Нет** гостевого залога. Вы **peer**. [Свой хаб](https://github.com/alexar76/aicom/blob/main/docs/join-the-federation.ru.md). |
| **Подключить свой MCP** только к своему ARGUS | allow-list / MCP-конфиг **вашего** агента | Локальная политика — **не** допуск в каталог Hub. |

Крутой агент на GitHub может **потреблять** AIMarket через MCP/ARGUS без THEMIS. Попасть в общий каталог, чтобы вам платили незнакомцы — жёсткий путь, и это **две двери**, не одна:

- **Гостевой издатель** на существующем Hub (эта страница): токен оператора + слэшируемый залог, prod ≈ **$25** (`POST /ai-market/v2/supply/stake`). $25 — залог, не плата за строку в каталоге.
- **Свой Hub** как peer федерации: [войти в федерацию](https://github.com/alexar76/aicom/blob/main/docs/join-the-federation.ru.md). Announce бесплатный; допуск по тому, что хаб *делает*. Это не бесплатный гостевой листинг на чужом узле.

Покупать уже листинговые cap — ни свой хаб, ни $25 не нужны.

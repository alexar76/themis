...
def _runtime_violation_state(
    ...
    * a report is bound to the digest of the declaration it contradicts, and
      ...
      Publishing an honest declaration retires stale reports.

    What this does NOT establish — stated because the previous wording claimed it did
    ("one hostile observer moves nothing"): distinct KEYS are not distinct OBSERVERS.
    An Ed25519 keypair costs nothing, so one actor with two of them satisfies a
    threshold of two. This service is stateless and non-authoritative — the caller
    supplies ``runtime_violations`` and gets back a signed opinion about *that input* —
    so the Sybil resistance has to live in whoever COLLECTED the reports and binds each
    reporter to a paid or registered identity (see the hub's
    ``supply_permission_violation``, which counts authenticated reporters, not keys).
    ``reporter_keys`` in the returned state is therefore reported as what it is: a key
    count. A consumer that treats it as an observer count is trusting the submitter.
    """
    ...
    if reported:
        _finding(
            ...
            "medium",
            "Runtime reports contradict the declaration but come from too few signing keys: "
            + ", ".join(reported)
            + ".",
            "Investigate before the next admission; a second signing key makes this fatal — "
            "and keys are free, so bind reporters to identities before acting on the count.",
            "ASI02",
            "ASI09",
        )
    return {
        ...
        "reported": reported,
        # Per permission: how many distinct issuer KEYS signed a contradicting report.
        # Named for what it measures so a consumer cannot mistake it for an observer
        # count — see the docstring.
        "reporter_keys": {
            permission: len(keys) for permission, keys in sorted(reporters.items())
        },
        "reporter_identity": "unbound",
    }
...
def audit(payload: AuditInput) -> dict[str, Any]:
    ...
    return {
        ...
        "attestations": {
            ...
            # Distinct signing KEYS per permission, and an explicit statement that key
            # distinctness is not observer distinctness — an Ed25519 keypair is free, and
            # this service is stateless so it cannot bind a reporter to an identity. A
            # consumer acting on the threshold owns that binding.
            "runtime_violation_reporter_keys": runtime_state["reporter_keys"],
            "runtime_violation_reporter_identity": runtime_state["reporter_identity"],
        },
        ...
        "limitations": [
            ...
            "compliance.",
            "Observer reports are counted by distinct signing key, and a signing key is "
            "free to create: the threshold proves signature diversity, not independent "
            "observers. Bind each reporter to a paid or registered identity before acting "
            "on runtime_violations_contradicting.",

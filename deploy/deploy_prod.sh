...
#
# The Hub gets the same inputs as scripts/deploy_hub.sh: the captured env filtered
# through scripts/security/service_env.py hub, and the read-only catalog export
# (scripts/security/refresh_factory_export.py) at /factory_data, never the Factory
# data tree, which holds secrets/ and signing keys. Both the deploy and the rollback.
#
...
NET="aicom_aicom_net"
# Where refresh_factory_export.py writes the catalog on the server (deploy_hub.sh's default).
FACTORY_EXPORT="${AIMARKET_FACTORY_EXPORT_ROOT:-/var/lib/aicom/hub-catalog}"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"

# The env filter and the catalog export that both the deploy and the rollback run.
sync_security_scripts() {
  rsync -az --exclude '__pycache__' "$ROOT/scripts/security/" "$HOST:$REMOTE/scripts/security/"
}
...
if [ "$MODE" = "rollback" ]; then
  ...
  ssh "$HOST" "set -e
    ...
    python3 scripts/security/service_env.py hub deploy-admission/hub.env.previous deploy-admission/hub.env.rollback
    # A rollback must not depend on the Factory being up: an earlier snapshot will do.
    python3 scripts/security/refresh_factory_export.py --destination $FACTORY_EXPORT \
      || test -f $FACTORY_EXPORT/state/pipeline.json
    ...
    docker run -d --name modelmarket-hub --restart unless-stopped \
      ...
      --env-file deploy-admission/hub.env.rollback \
      \"\$previous\"
    sleep 6
...
ssh "$HOST" "set -e
  ...
  python3 scripts/security/service_env.py hub deploy-admission/hub.env.previous deploy-admission/hub.env.base
  grep -vE '^AIMARKET_SUPPLY_CHAIN_' deploy-admission/hub.env.base > deploy-admission/hub.env
  rm -f deploy-admission/hub.env.base
  {
...
ssh "$HOST" "set -e
  ...
  # Before anything is removed: a failed export leaves the running hub as it is.
  python3 scripts/security/refresh_factory_export.py --destination $FACTORY_EXPORT

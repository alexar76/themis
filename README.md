...
<p align="center">
  ...
  <a href="https://github.com/alexar76/themis/blob/main/LICENSE"><img src="https://raw.githubusercontent.com/alexar76/themis/refs/heads/main/docs/badges/license.svg" alt="License: MIT" /></a>
</p>
<!-- /aicom-readme-badges -->
...
<p align="center">
  ...
  <sub><b>Should this AI agent be allowed into your business?</b> —
    ...
    <a href="https://alexar76.github.io/themis/console/"><b>dashboards / console →</b></a> ·
    ...
    <a href="https://github.com/alexar76/themis/blob/main/docs/admission/en.md"><b>Hub admission →</b></a>
  </sub>
</p>
...
Receipt cards below are **screenshots of the live admission console** at
[`/ui/`](http://127.0.0.1:8080/ui/) after a real local `/invoke` (safe → **approve**, unsafe → **reject**).
Capability id: `agent.security.supply-chain.audit@v1`.

**Public dashboards** (no factory admin):

| Surface | URL |
|---|---|
| Landing | https://alexar76.github.io/themis/ |
| Admission console (Pages) | https://alexar76.github.io/themis/console/ |
| Alien Monitor (telemetry) | https://monitor.modelmarket.dev/ |
| Local operator UI | http://127.0.0.1:8080/ui/ after `uv run python agent.py` |
...
THEMIS. See [supply-chain admission](https://github.com/alexar76/themis/blob/main/docs/admission/en.md).
...
uv run python agent.py
# open http://127.0.0.1:8080/ui/  — live admission console
```
...
The safe sample returns `decision: approve`. Open `/ui/`, load **unsafe example**, and observe a
fail-closed **REJECT** receipt with live findings.
...
- an evidence reference counts toward the buyer's evidence floor only when it is bound to a
  SHA-256 digest, so decorative links cannot satisfy procurement;
- attestations are verified offline against their own canonical statement — never by fetching;
- a signature that does not verify is a finding, while a merely absent one is scored only when
  policy asks for it;
...
- Metis job state is per-process by default; `METIS_JOB_DB` moves it to a shared store so any
  replica can answer any poll, and `THEMIS_REPLICAS>1` without one refuses to start;
...
A URL is a claim. A URL with a digest identifies an artifact. A URL with a digest **and** a
signature is a claim someone put their key behind — the only one of the three this service can
check without fetching anything.

```bash
uv run python attest.py evidence --kind sbom \
  --url https://vendor.example.com/sbom.json --sha256 <64-hex>
uv run python attest.py permissions --dossier examples/safe_candidate.json
```

Paste the printed object into the dossier as `evidence[].attestation` or
`permissions_attestation`. The report's `attestations` block then records exactly what was
verified: how many references were signed, whether any issuer was independent of the candidate,
and whether the permission declaration is bound to `provider_pubkey`.

| Policy field | Default | Effect |
|---|---|---|
| `require_evidence_digests` | `true` | Undigested references never count toward the evidence floor |
| `require_evidence_attestation` | `false` | Every counted reference needs a verified signature |
| `require_independent_attestation` | `false` | At least one issuer must not be the candidate's own key |
| `require_permission_attestation` | `false` | The declaration must be signed by `provider_pubkey` |
| `trusted_evidence_issuers` | `[]` | When set, only these issuer keys satisfy an attestation |

Declared permissions remain a declaration — this does not make them true. It makes a false one
signed, attributable and therefore slashable.

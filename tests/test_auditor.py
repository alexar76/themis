...
# ── 2026-08 audit: the runtime-violation threshold counts KEYS, not observers ──


def test_report_states_that_reporter_identity_is_unbound(safe_input):
    """The docstring used to claim "one hostile observer moves nothing".

    It does not hold: an Ed25519 keypair is free, so one actor with two of them meets a
    threshold of two. THEMIS is stateless and cannot bind identity, so the report must SAY
    that rather than imply observer independence — the consumer owns that check.
    """
    report = audit(safe_input)
    att = report["attestations"]
    assert att["runtime_violation_reporter_identity"] == "unbound"
    assert isinstance(att["runtime_violation_reporter_keys"], dict)
    assert any(
        "signing key is free" in line or "free to create" in line
        for line in report["limitations"]
    ), "the limitation must be stated to the consumer, not only in a docstring"

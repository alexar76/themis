...
def test_admission_console_is_served_and_backed_by_live_invoke():
    with TestClient(agent.app) as client:
        ...
        assert "ADMISSION RECEIPT" in page.text
        assert "Run admission" in page.text
        safe = client.get("/examples/safe_candidate.json")
        unsafe = client.get("/examples/unsafe_candidate.json")
        unknown = client.get("/examples/nope.json")
    assert safe.status_code == 200
    assert unsafe.status_code == 200
    assert unknown.status_code == 404
    with TestClient(agent.app) as client:
        reject = client.post("/invoke", json=unsafe.json())
    assert reject.status_code == 200
    assert reject.json()["result"]["decision"] == "reject"
